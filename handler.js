    ...
    async participantsUpdate({ id, participants, action }) {
        ...
        switch (action) {
            ...
            case 'remove':
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc.toString()) :
                                (chat.sBye || this.bye || conn.bye || 'Bye, @user!')).replace('@user', '@' + user.split('@')[0]).replace('@subject', await this.getName(id))
                ...
                break
        case 'ytmp4': case 'ytv': {
if (!q) throw `Example : ${prefix + command} https://www.youtube.com/watch?v=ysdDkJf0kfI 480p`
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        premium: '*Undang Bot ke Grup+Premium*\n1 Bulan *Rp20.000*\n2 Bulan *Rp40.000*\n\nKetik .sewa kalau minat..', 
