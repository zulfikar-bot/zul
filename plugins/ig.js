...
instagramdl(args[0]).then(async res => {
    ...
    for (let { url, type } of json) {
      await delay(1500)
      conn.sendFile(m.chat, url, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), 'Zulfikar Bot', m, { thumbnail: Buffer.alloc(0) })
    }
  })
}
handler.help = ['ig'].map(v => v + ' <url>')
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  m.reply('Sedang diproses...')
   conn.sendFile(m.chat, json.linkdownload, 'ig.mp4', '*© Zulfikar Bot*', m, { thumbnail: Buffer.alloc(0) })
}
