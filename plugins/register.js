...
let handler = async function (m, { conn, text, usedPrefix }) {
  ...
  let expnye = global.db.data.users[m.sender].exp += 2500
  let uang = global.db.data.users[m.sender].money += 25000
  let limitnye = global.db.data.users[m.sender].limit += 25
// let nabungnye = global.db.data.users[m.sender].nabung += 100000
  let banknye = global.db.data.users[m.sender].bank += 25000
