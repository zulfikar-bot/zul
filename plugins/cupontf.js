let handler = async (m, { conn, args, usedPrefix, owner }) => {
    if (args.length < 3) {
        return conn.reply(m.chat, `Gunakan format ${usedPrefix}tfcupon <cupon> <jumlah> <@tag>\ncontoh penggunaan: *${usedPrefix}tfcupon cupon 1 @tag*`.trim(), m)
    } else try {
        ...
        switch (type) {
            ...
            default:
                return conn.reply(m.chat, `Gunakan format ${usedPrefix}tfcupon <type> <jumlah> <@tag>\ncontoh penggunaan: *${usedPrefix}tfcupon cupon 1 @tag*\n\n*List yang bisa di tfcupon*\ncupon`.trim(), m)
        }
    } catch (e) {
        conn.reply(m.chat, `Format yang anda gunakan salah\n\nGunakan format ${usedPrefix}tfcupon <type> <jumlah> <@tag>\ncontoh penggunaan: *${usedPrefix}tfcupon cupon 1 @tag*`.trim(), m)
        console.log(e)
        if (owner) {
            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn.user.jid)) {
                conn.reply(jid, 'Transfer.js error\nNo: *' + m.sender.split`@`[0] + '*\nCommand: *' + m.text + '*\n\n*' + e + '*', m)
